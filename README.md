...
字段名称和说明只作为普通文字显示，不会被当作 HTML 解析。
...
| `@row` | `@columns`、`@bytes-per-row` | 否 | 每行显示的字节数，默认 `16` |
...
- 单字节位号范围是 `0..7`。
- 推荐按常见阅读顺序写成 `7..4`、`3..0`。
- 插件也接受 `4..7`，显示时仍按 bit 7 → bit 0 排列。
- 位域之间不能重叠。
- 未声明的 bit 会显示为“未定义”。
- 顶部 `bit 7..bit 0` 序号格直接使用所属位域的颜色。
...
- 多字节子字段不能相互重叠。
- 多字节位图会显示真实字节序号、每字节 bit 7 → bit 0，以及各子字段的彩色跨度。
